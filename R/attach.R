...
pkgs <- c("ggplot2", "dplyr", "rmarkdown", "roadoi", "yaml", "httr", ,    "pdftools", "RSelenium", "workflowr", "stringr", "tidyr", ,    "glmnet", "MASS", "stargazer", "rvest", "stringr", "quanteda", ,    "RColorBrewer", "rcrossref", "fulltext", "pubchunks", "devtools", ,    "roxygen2")
...
iuneverse_attach <- function() {
  ...
  # iuneverse with the package version on the right-hand side
  load_header <- cli::rule(
    left = crayon::bold("Attaching packages"),
    right = paste0("iuneverse ", package_version("iuneverse"))
  )

  # Return a character string containing the package version for each of iuneverse's constituents
...
iuneverse_detach <- function() {
  pak <- paste0("package:", c(pkgs, "iuneverse"))
  lapply(pak[pak %in% search()], detach, character.only = TRUE)
  invisible()
}

#' List all packages imported by iuneverse
#'
#' @export
#'
#' @examples
#' iuneverse_packages()
iuneverse_packages <- function() {
  # get all imports from iuneverse's package description file
  raw <- utils::packageDescription("iuneverse")$Imports
